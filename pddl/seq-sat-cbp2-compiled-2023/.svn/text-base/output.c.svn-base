...

{
  ...
  if ( !a->norm_operator &&
    ...
    fprintf(output_file,"%s", a->name ); 
    for ( i = 0; i < a->num_name_vars; i++ ) {
      fprintf(output_file," %s", gconstants[a->name_inst_table[i]]);
    }
  }
...
{  
  ...
  for ( i = 0; i < gnum_plan_ops; i++ ) {
   ...
   if (gcmd_line.o_file){
      sprintf(output_file_name, "%s%s%s%d", gcmd_line.path, gcmd_line.ofile_name,".",nsolutions);
	...
	{
	  fprintf(output_file,";NActions: %d\n",gnum_plan_ops );
	  fprintf(output_file,";Time: %7.2f\n", gtempl_time + greach_time + grelev_time + gLNF_time + gconn_time + gsearch_time );
	  fprintf(output_file,";Cost: %7.2f\n", final_total_cost );
	  for ( i = 0; i < gnum_plan_ops; i++ ) {
	    fprintf(output_file,"%4d: ", i);
	    fprintf(output_file,"(");
	    print_op_name_to_file( gplan_ops[i], output_file );
	    fprintf(output_file,")");
	    fprintf(output_file,"\n");
	  }
