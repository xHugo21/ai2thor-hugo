...

{
  ...
  if (gcmd_line.search_algorithm == 10 || gcmd_line.search_algorithm == 12 || gcmd_line.search_algorithm == 13 || gcmd_line.search_algorithm == 16 || gcmd_line.search_algorithm == 17  || gcmd_line.search_algorithm == 18  ){
...

{
  ...
  printf("                3: hmax \n");
  printf("                4: hadd \n");
  printf("                5: hadd with helpful actions \n");
  printf("                6: hmax withhelpful actions \n");
  printf("                7: sum-hmax \n");
  printf("                8: max-hadd \n");
  printf("                9: sum-hmax with helpful actions \n");
  printf("                10:max-hadd with helpful actions \n\n");
  ...
  printf("                19: CEHC ordering first level by h(n)+ cost(a)\n");
  printf("                20: CEHC hdiff \n");
  printf("                21: random search \n\n");
  ...
  printf("-c <num> horizon for the BFS Roller policy algorithm (-S 12,13,17,18) (default 50 ) \n");
  printf("-s <num> seed \n");
...

{
  ...
  gcmd_line.hdiff = 0;
  gcmd_line.roller_bfs_horizon = 50;
  
  final_total_cost = INFINITY;
  ...
  while ( --argc && ++argv ) {
	...
	case 'H':
	  sscanf( *argv, "%d", &gcmd_line.heuristic);
          if (gcmd_line.heuristic < 0 || gcmd_line.heuristic > 10)
	    {
	...
	case 'S':
	  sscanf( *argv, "%d", &gcmd_line.search_algorithm);
          if (gcmd_line.search_algorithm < 0 || gcmd_line.search_algorithm > 21)
	    {
	...
	case 'y':
	  sscanf( *argv, "%f", &final_total_cost);
	  break;
...
//   printf("\n Estoy aquí!");
//  printf("\n Search Tree: First node children %d", gSearch_tree->num_children);
...

{
  ...
  */
  ...
  switch (gcmd_line.search_algorithm) {
    ...
    case 3 : found_plan = do_anytime_best_first_search(); break; 
 ...
 case 15 : { 
   found_plan = do_best_first_search();
   printf("\n First solution found at %d depth...\n performing exhaustive search", gnum_plan_ops);
   if ( found_plan ) {
     Search_upper_bound = gnum_plan_ops + 1 ;
     found_plan = do_anytime_bfs_bnb_learn();
//      execute_learner();
   }
   break;
  ...
  case 19 : found_plan = do_enforced_hill_climbing_cost_order_first_level();
  case 20 : found_plan = do_enforced_hill_climbing_cost_hdiff(); break;
  case 21 : 
      {
  ...
  if (gcmd_line.search_algorithm == 0 || gcmd_line.search_algorithm == 1 || gcmd_line.search_algorithm == 2 || gcmd_line.search_algorithm == 19 || gcmd_line.search_algorithm == 20  )
    {
    if ( found_plan ) {
      print_plan();
    }
